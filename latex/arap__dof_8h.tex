...
Arap DOF precomputation consists of two parts the computation. The first is that which depends solely on the mesh (V,F), the linear blend skinning weights (M) and the groups G. Then there\textquotesingle{}s the part that depends on the previous precomputation and the list of free and fixed vertices.\hypertarget{arap__dof_8h_autotoc_md3}{}\doxyparagraph{\texorpdfstring{Caller example\+:}{Caller example\+:}}\label{arap__dof_8h_autotoc_md3}
\begin{DoxyVerb}Once:
